...
app.post("/terminal/razorpay/create-order", async (req, res) => {
  try {
    ...
    const amountINR = Number(parcel.cost ? parcel.cost.toString() : 0);
    ...
    // Save razorpay order id on parcel
...
app.post("/terminal/razorpay/verify", async (req, res) => {
  try {
    ...
    // Signature ok -> update parcel as paid
    const parcel = await Parcel2.findById(parcelId);
    if (!parcel) return res.status(404).json({ error: "Parcel not found" });

    parcel.razorpayOrderId = razorpay_order_id;
    parcel.razorpayPaymentId = razorpay_payment_id;
    parcel.razorpaySignature = razorpay_signature;
    parcel.paymentStatus = "completed";
    await parcel.save();
    // Respond success — client can redirect to access page
    return res.json({ success: true, parcelId: parcel._id });
  } catch (err) {
